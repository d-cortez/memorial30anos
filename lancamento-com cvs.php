<?php
	...
	if (file_exists("importacao/teste.csv")){
		$handle = fopen ("importacao/teste.csv","r");
		$row = 1;
		while (($data = fgetcsv($handle, 1000, ";")) !== FALSE) {
			$importacao[$row] = iconv('ISO-8859-1', 'UTF-8', $data[0]).";".
								iconv('ISO-8859-1', 'UTF-8', $data[1]);
			$row++;
		}
		fclose ($handle);
		unlink ($handle);
				...
				<?if (!$EncontroDigitado) {?>
				  	...
				  	<table class="table table-striped tabl$Encontro-hover">
				  		...
				  		if ($importacao) {
				  			...
				  			foreach ($importacao as $dado) {

				  				$seguimista = explode(";", $dado);
								$nome 	= $seguimista[0];
								$cracha = $seguimista[1];

				  				echo '<tr>';
					  			echo '<td>'.$i.'</td>';
					            echo "<td><input style='width:100%' value='$nome'		type=text name=txtNome[$i]  	id=txtNome[$i] 		placeholder='Nome completo do seguidor #".$i."'></td>";
					            echo "<td><input style='width:100%' value='$cracha' 	type=text name=txtCracha[$i]    id=txtCracha[$i] 	placeholder='Nome do crachá #".$i."'></td>";			            
					            echo '</tr>';
				  			 	$i++;
				  			}
